...
import { CaseProposalListDocument, useCaseProposalListQuery, useUpdateCaseRequestMutation, CaseRequestStatus as CaseRequestStatusGQL } from "../../../../graphql-api";
...
const useProposalList = () => {
  ...
  const proposals = data?.caseRequests.edges.map((edge) => ({
    id: edge.node.id,
  ...
  const acceptProposal = (id: string) => {
    update({
      ...
      onCompleted: () => {
        success(`Case proposal ${status.toLowerCase()}`);
      },
      onError: (err) => {
        error(err.message);
      },
